...
const deselectTownBtn = document.getElementById("deselectTownBtn") as HTMLButtonElement;
...
function rebuildTownDropdown() {
  ...
  const townsWithStations = townsData.features.filter((town) =>
    stationsData!.features.some((st) => turf.booleanPointInPolygon(st.geometry, town.geometry)),
  );
...
function updateStationsColor() {
  ...
  const mode = (document.querySelector('input[name="availability"]:checked') as HTMLInputElement)
    .value;
...
function updateStats() {
  ...
  if (selectedTownId && townsData) {
    const townFeature = townsData.features.find((f) => f.properties!.TOWN_ID == selectedTownId);
    if (townFeature) {
  ...
  const title = formattedTownName ? `Stats for ${formattedTownName}` : "Stats for All Stations";

  const num = filtered.length;
  const bikes = filtered.reduce((sum, s) => sum + Number(s.properties!.bikesAvailable), 0);
  const ebikes = filtered.reduce((sum, s) => sum + Number(s.properties!.ebikesAvailable), 0);
  const docks = filtered.reduce((sum, s) => sum + Number(s.properties!.bikeDocksAvailable), 0);
